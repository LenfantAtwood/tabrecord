...
4. Guitar Pro 兼容只以 GP8 为目标，并优先完成输入。alphaTab 可承担 GP8 读取与渲染；GP8 写回必须单独验证，不能把旧版 exporter 当作完成依据。

## 2. 项目与资料

| 项目/论文 | 可学习内容 | 与 TabRecord 的关系 | 主要注意点 |
| --- | --- | --- | --- |
| [alphaTab](https://github.com/CoderLine/alphaTab) / [GP8 格式文档](https://alphatab.net/docs/formats/guitar-pro-8/) | GP8 输入、数据模型、SVG/Canvas TAB 渲染与播放 | GP8 输入和页面渲染适配器 | SDK 而非完整编辑器；现有 exporter 不能直接证明 GP8 写回；MPL-2.0 |
...
## 3. GP8 与 alphaTab 能力边界

官方资料显示：

- alphaTab 可以读取 GP8 `.gp` 的相应支持子集；
- 可以渲染标准谱和 TAB、播放并显示光标；
- alphaTab 明确定位为构建记谱软件的 SDK，而不是开箱即用编辑器。

建议：

- GP8 是唯一 Guitar Pro 兼容边界，不为其他 Guitar Pro 版本建立独立适配器；
- P0 只要求 GP8 输入、页面映射、渲染和播放；
- GP8 写回在输入模型稳定后实现，并必须由 Guitar Pro 8 重开及语义 diff 验证；
- TabRecord 维护自己的修订历史和页面乐谱，alphaTab 只位于输入/渲染适配层；
- 在 spike 中验证修改内存 `Score` 后的重渲染成本，以及是否能只刷新脏小节；
- 只建立 GP8 字段兼容矩阵，不写“全格式无损”。
...
- 每个推理窗口直接追加音符，造成边界重复；
- 在录制头上每帧全量重绘整首谱；
- 把模型生成的未确认内容写入最终 GP8 后再试图恢复历史。

## 7. 建议先复现的三个基线

1. **Baseline A**：Basic Pitch → note events → 固定 BPM 量化 → 最小换把动态规划 → 页面乐谱/MIDI。
